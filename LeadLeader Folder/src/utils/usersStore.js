...
async function createUser({username,passHash,role='user',plan='free'}){const db=await readAll(); if(db.users.some(u=>u.username.toLowerCase()===username.toLowerCase())){const e=new Error('User exists');e.code='E_EXISTS';throw e} const id=`u_${Math.random().toString(36).slice(2)}`; const user={id,username,passHash,role,plan,createdAt:nowIso()}; db.users.push(user); await writeAll(db); return {id,username,role,plan,createdAt:user.createdAt}}
async function setRole(username,role){const db=await readAll(); const u=db.users.find(x=>x.username.toLowerCase()===username.toLowerCase()); if(!u){const e=new Error('Not found');e.code='E_NOTFOUND';throw e} u.role=role; await writeAll(db); return {id:u.id,username:u.username,role:u.role,plan:u.plan||'free',createdAt:u.createdAt}}
async function setPlan(username,plan){const db=await readAll(); const u=db.users.find(x=>x.username.toLowerCase()===username.toLowerCase()); if(!u){const e=new Error('Not found');e.code='E_NOTFOUND';throw e} u.plan=plan; await writeAll(db); return {id:u.id,username:u.username,role:u.role,plan:u.plan,createdAt:u.createdAt}}
async function getPlan(username){const u=await getByUsername(username); return u?.plan||'free'}
async function renameUser(oldName,newName){const db=await readAll(); const u=db.users.find(x=>x.username.toLowerCase()===oldName.toLowerCase()); if(!u){const e=new Error('Not found');e.code='E_NOTFOUND';throw e} if(db.users.some(x=>x.username.toLowerCase()===newName.toLowerCase())){const e=new Error('User exists');e.code='E_EXISTS';throw e} u.username=newName; await writeAll(db); return {id:u.id,username:u.username,role:u.role,plan:u.plan||'free',createdAt:u.createdAt}}
async function resetPassword(username,passHash){const db=await readAll(); const u=db.users.find(x=>x.username.toLowerCase()===username.toLowerCase()); if(!u){const e=new Error('Not found');e.code='E_NOTFOUND';throw e} u.passHash=passHash; await writeAll(db); return {id:u.id,username:u.username,role:u.role,plan:u.plan||'free',createdAt:u.createdAt}}
...
module.exports={initStore,getByUsername,listAll,createUser,setRole,setPlan,getPlan,renameUser,resetPassword,deleteUser,USERS_FILE:USERS_FILE,DATA_DIR:DATA_DIR}
